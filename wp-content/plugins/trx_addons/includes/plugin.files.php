...
if ( !function_exists( 'trx_addons_merge_sass' ) ) {
	function trx_addons_merge_sass($to, $list, $need_responsive=false, $root='../') {
		...
		if ($need_responsive) $responsive = (array) apply_filters('trx_addons_filter_sass_responsive', $TRX_ADDONS_STORAGE['responsive']);
		$sass = array(
			'import' => '',
			'sizes'  => array()
			);
				...
				if ($need_responsive) {
					...
					foreach ($responsive as $k=>$v) {
						if (preg_match("/([\d\w\-_]+--{$k})/", $css, $matches)) {
							$sass['sizes'][$k] = (!empty($sass['sizes'][$k]) ? $sass['sizes'][$k] : '') . "\t@include {$matches[0]}();\n";
							$add = true;
						}
					}
				} else
