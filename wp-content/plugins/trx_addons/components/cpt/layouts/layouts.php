...
if ( !function_exists( 'trx_addons_cpt_layouts_merge_styles_for_export' ) ) {
	...
	add_action( 'trx_addons_action_importer_import_end', 'trx_addons_cpt_layouts_merge_styles_for_export', 20 );
	add_action( 'activated_plugin', 'trx_addons_cpt_layouts_merge_styles_for_export', 20 );
	...
	function trx_addons_cpt_layouts_importer_export($importer) {
		...
		if (is_array($posts) && count($posts) > 0) {
			...
			foreach ($posts as $post) {
				...
				$panels_data = serialize(get_post_meta( $post->ID, 'panels_data', true ));
				...
				$output .= ($counter++ ? ',' : '') 
						...
						. "\n\t\t\t\t\t\t'trx_addons_options' => '{$trx_addons_options}',"
