...
if (!isset($trx_addons_args['link'])) $trx_addons_args['link'] = true;
		...
		if ( $trx_addons_args['singular'] ) {
			...
			if ( has_post_thumbnail() ) {
					...
					if (!empty($trx_addons_args['hover']) && $trx_addons_post_format != 'audio') {
						...
						if ($trx_addons_args['hover'] == 'zoomin') {
							...
							?><div class="trx_addons_hover_content">
								<a href="<?php the_permalink(); ?>" class="trx_addons_hover_icon trx_addons_hover_icon_link"></a>
				...
				if (!empty($trx_addons_image)) {
					...
					?></a><?php trx_addons_show_layout($trx_addons_post_info); ?></div><?php
				}
