...
------------------------------------------------------------------------------- */
if (!function_exists('gamezone_trx_addons_theme_specific_setup1')) {
	add_filter( 'trx_addons_filter_components_editor', 'gamezone_trx_addons_theme_specific_components');
	function gamezone_trx_addons_theme_specific_components($enable=false) {
		return false;
	}
}

if (!function_exists('gamezone_trx_addons_theme_specific_setup1')) {
	add_action( 'after_setup_theme', 'gamezone_trx_addons_theme_specific_setup1', 1 );
	function gamezone_trx_addons_theme_specific_setup1() {
		if (gamezone_exists_trx_addons()) {
			add_filter( 'trx_addons_cv_enable',					'gamezone_trx_addons_cv_enable');
			add_filter( 'trx_addons_demo_enable',				'gamezone_trx_addons_demo_enable');
			add_filter( 'trx_addons_filter_edd_themes_market',	'gamezone_trx_addons_edd_themes_market_enable');
			add_filter( 'trx_addons_cpt_list',					'gamezone_trx_addons_cpt_list');
			add_filter( 'trx_addons_sc_list',					'gamezone_trx_addons_sc_list');
			add_filter( 'trx_addons_widgets_list',				'gamezone_trx_addons_widgets_list');
		}
	}
}

// CV
if ( !function_exists( 'gamezone_trx_addons_cv_enable' ) ) {
	
	function gamezone_trx_addons_cv_enable($enable=false) {
		// To do: return false if theme not use CV functionality
		return GAMEZONE_THEME_FREE
					? false		// Free version
					: true;		// Pro version
	}
}
...
// Demo mode
if ( !function_exists( 'gamezone_trx_addons_demo_enable' ) ) {
	
	function gamezone_trx_addons_demo_enable($enable=false) {
...
// EDD Themes market
if ( !function_exists( 'gamezone_trx_addons_edd_themes_market_enable' ) ) {
	
	function gamezone_trx_addons_edd_themes_market_enable($enable=false) {
...
// API
if ( !function_exists( 'gamezone_trx_addons_api_list' ) ) {
	
	function gamezone_trx_addons_api_list($list=array()) {
...
// CPT
if ( !function_exists( 'gamezone_trx_addons_cpt_list' ) ) {
	
	function gamezone_trx_addons_cpt_list($list=array()) {
...
// Shortcodes
if ( !function_exists( 'gamezone_trx_addons_sc_list' ) ) {
	
	function gamezone_trx_addons_sc_list($list=array()) {
...
// Widgets
if ( !function_exists( 'gamezone_trx_addons_widgets_list' ) ) {
	
	function gamezone_trx_addons_widgets_list($list=array()) {
		...
		$recent_news = array('recent_news');
		foreach ($list as $k=>$v) {
			if (in_array($k, $recent_news)) {
				$list[$k]['layouts_sc']['news-modern'] = esc_html__('Modern', 'gamezone');
				$list[$k]['layouts_sc']['news-announce-modern'] = esc_html__('Announce Modern', 'gamezone');
				$list[$k]['layouts_sc']['news-announce-full'] = esc_html__('Announce Full', 'gamezone');
				$list[$k]['layouts_sc']['news-modern-right'] = esc_html__('Modern Right', 'gamezone');
				$list[$k]['layouts_sc']['news-modern-left'] = esc_html__('Modern Left', 'gamezone');
				$list[$k]['layouts_sc']['news-modern-excerpt'] = esc_html__('Modern Excerpt', 'gamezone');
			}
		}
...
if ( !function_exists( 'gamezone_trx_addons_menu_cache' ) ) {
	add_filter( 'trx_addons_filter_menu_cache', 'gamezone_trx_addons_menu_cache');
	function gamezone_trx_addons_menu_cache($list=array()) {
...
if (!function_exists('gamezone_trx_addons_localize_script')) {
	...
	function gamezone_trx_addons_localize_script($arr) {
		$arr['alter_link_color'] = gamezone_get_scheme_color('alter_link');
		return $arr;
	}
}
...
if ( !function_exists( 'gamezone_trx_addons_sc_type' ) ) {
	add_filter( 'trx_addons_sc_type', 'gamezone_trx_addons_sc_type', 10, 2);
	function gamezone_trx_addons_sc_type($list, $sc) {
		// To do: check shortcode slug and if correct - add new 'key' => 'title' to the list

		if ($sc == 'trx_sc_title') {
			$list['modern'] = esc_html__('Modern', 'gamezone');		}
...
if ( !function_exists( 'gamezone_trx_addons_sc_atts' ) ) {
	add_filter( 'trx_addons_sc_atts', 'gamezone_trx_addons_sc_atts', 10, 2);
	function gamezone_trx_addons_sc_atts($atts, $sc) {
...
if ( !function_exists( 'gamezone_trx_addons_sc_map' ) ) {
	add_filter( 'trx_addons_sc_map', 'gamezone_trx_addons_sc_map', 10, 2);
	function gamezone_trx_addons_sc_map($params, $sc) {
		...
		$param = array(
			...
			'heading' => esc_html__('Color style', 'gamezone'),
			'description' => wp_kses_data( __('Select color style to decorate this block', 'gamezone') ),
			'edit_field_class' => 'vc_col-sm-4',
			'admin_label' => true,
			'value' => array_flip(gamezone_get_list_sc_color_styles()),
			'type' => 'dropdown'
		);
...
if ( !function_exists( 'gamezone_trx_addons_sow_map' ) ) {
	add_filter( 'trx_addons_sow_map', 'gamezone_trx_addons_sow_map', 10, 2);
	function gamezone_trx_addons_sow_map($params, $sc) {

		// Param 'color_style'
		$param = array(
			'color_style' => array(
				'label' => esc_html__('Color style', 'gamezone'),
				'description' => wp_kses_data( __('Select color style to decorate this block', 'gamezone') ),
				'options' => gamezone_get_list_sc_color_styles(),
				'default' => 'default',
				'type' => 'select'
			)
		);
		if (in_array($sc, array('trx_sc_button')))
			gamezone_array_insert_after($params, 'size', $param);
		else if (in_array($sc, array('trx_sc_action', 'trx_sc_blogger', 'trx_sc_cars', 'trx_sc_courses', 'trx_sc_content', 'trx_sc_dishes',
								'trx_sc_events', 'trx_sc_form',	'trx_sc_googlemap', 'trx_sc_portfolio', 'trx_sc_price', 'trx_sc_promo',
								'trx_sc_properties', 'trx_sc_services', 'trx_sc_team', 'trx_sc_testimonials', 'trx_sc_title',
								'trx_widget_audio', 'trx_widget_twitter')))
			gamezone_array_insert_after($params, 'title_align', $param);
		return $params;
	}
}

// Add classes to the shortcode's output
if ( !function_exists( 'gamezone_trx_addons_sc_output' ) ) {
	add_filter( 'trx_addons_sc_output', 'gamezone_trx_addons_sc_output', 10, 4);
	function gamezone_trx_addons_sc_output($output, $sc, $atts, $content) {
		...
		if (in_array($sc, array('trx_sc_action'))) {
			...
			if (!empty($atts['color_style']) && !gamezone_is_inherit($atts['color_style']))
...
if ( !function_exists( 'gamezone_trx_addons_sc_item_title_tag' ) ) {
	add_filter( 'trx_addons_filter_sc_item_title_tag', 'gamezone_trx_addons_sc_item_title_tag');
	function gamezone_trx_addons_sc_item_title_tag($tag='') {
		return $tag=='h1' ? 'h2' : $tag;
	}
}

// Return args for the item's button
if ( !function_exists( 'gamezone_trx_addons_sc_item_button_args' ) ) {
	add_filter( 'trx_addons_filter_sc_item_button_args', 'gamezone_trx_addons_sc_item_button_args', 10, 3);
	function gamezone_trx_addons_sc_item_button_args($args, $sc, $sc_args) {
...
if ( !function_exists( 'gamezone_trx_addons_slider_title' ) ) {
	add_filter( 'trx_addons_filter_slider_title',	'gamezone_trx_addons_slider_title', 10, 2 );
	function gamezone_trx_addons_slider_title($title, $data) {
		$title = '';
		if (!empty($data['title'])) 
...
if ( !function_exists( 'gamezone_trx_addons_sc_googlemap_styles' ) ) {
	add_filter( 'trx_addons_filter_sc_googlemap_styles',	'gamezone_trx_addons_sc_googlemap_styles');
	function gamezone_trx_addons_sc_googlemap_styles($list) {
		$list['dark'] = esc_html__('Dark', 'gamezone');
		return $list;
	}
}
...
if ( !function_exists( 'gamezone_trx_addons_tiny_mce_style_formats' ) ) {
	add_filter( 'trx_addons_filter_tiny_mce_style_formats', 'gamezone_trx_addons_tiny_mce_style_formats');
	function gamezone_trx_addons_tiny_mce_style_formats($style_formats) {
		...
		if (false && is_array($style_formats) && count($style_formats)>0 ) {
			foreach ($style_formats as $k=>$v) {
				if ( $v['title'] == esc_html__('List styles', 'gamezone') ) {
					$style_formats[$k]['items'][] = array(
								'title' => esc_html__('Arrow', 'gamezone'),
								'selector' => 'ul',
...
if ( !function_exists( 'gamezone_trx_addons_allow_override_header_image' ) ) {
	add_filter( 'gamezone_filter_allow_override_header_image', 'gamezone_trx_addons_allow_override_header_image' );
	function gamezone_trx_addons_allow_override_header_image($allow) {
		return gamezone_is_team_page() ? false : $allow;
	}
}

// Get thumb size for the team items
if ( !function_exists( 'gamezone_trx_addons_thumb_size' ) ) {
	add_filter( 'trx_addons_filter_thumb_size',	'gamezone_trx_addons_thumb_size', 10, 2);
	function gamezone_trx_addons_thumb_size($thumb_size='', $type='') {
...
// All other CPT may be modified in the same method
if (!function_exists('gamezone_trx_addons_override_options_fields')) {
	add_filter( 'trx_addons_filter_override_options_fields', 'gamezone_trx_addons_override_options_fields', 10, 2);
	function gamezone_trx_addons_override_options_fields($mb, $post_type) {
		if (defined('TRX_ADDONS_CPT_TEAM_PT') && $post_type==TRX_ADDONS_CPT_TEAM_PT) {
			$mb['email'] = array(
				"title" => esc_html__("E-mail",  'gamezone'),
				"desc" => wp_kses_data( __("Team member's email", 'gamezone') ),
				"std" => "",
				"details" => true,
				"type" => "text"
			);
