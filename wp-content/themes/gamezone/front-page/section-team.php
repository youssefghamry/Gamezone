<div class="front_page_section front_page_section_team<?php
			$gamezone_scheme = gamezone_get_theme_option('front_page_team_scheme');
			if (!gamezone_is_inherit($gamezone_scheme)) echo ' scheme_'.esc_attr($gamezone_scheme);
			echo ' front_page_section_paddings_'.esc_attr(gamezone_get_theme_option('front_page_team_paddings'));
		?>"<?php
		$gamezone_css = '';
		$gamezone_bg_image = gamezone_get_theme_option('front_page_team_bg_image');
...
?>><?php
	// Add anchor
	$gamezone_anchor_icon = gamezone_get_theme_option('front_page_team_anchor_icon');	
	$gamezone_anchor_text = gamezone_get_theme_option('front_page_team_anchor_text');	
	...
	<div class="front_page_section_inner front_page_section_team_inner<?php
			if (gamezone_get_theme_option('front_page_team_fullheight'))
				echo ' gamezone-full-height sc_layouts_flex sc_layouts_columns_middle';
			?>"<?php
			$gamezone_css = '';
			$gamezone_bg_mask = gamezone_get_theme_option('front_page_team_bg_mask');
			$gamezone_bg_color = gamezone_get_theme_option('front_page_team_bg_color');
	...
	?>>
		<div class="front_page_section_content_wrap front_page_section_team_content_wrap content_wrap">
			<?php
			// Caption
			$gamezone_caption = gamezone_get_theme_option('front_page_team_caption');
			if (!empty($gamezone_caption) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				?><h2 class="front_page_section_caption front_page_section_team_caption front_page_block_<?php echo !empty($gamezone_caption) ? 'filled' : 'empty'; ?>"><?php echo wp_kses($gamezone_caption, 'gamezone_kses_content'); ?></h2><?php
			}
		
			// Description (text)
			$gamezone_description = gamezone_get_theme_option('front_page_team_description');
			if (!empty($gamezone_description) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				?><div class="front_page_section_description front_page_section_team_description front_page_block_<?php echo !empty($gamezone_description) ? 'filled' : 'empty'; ?>"><?php echo wp_kses(wpautop($gamezone_description), 'gamezone_kses_content'); ?></div><?php
			}
			...
			?><div class="front_page_section_output front_page_section_team_output"><?php 
				if (is_active_sidebar('front_page_team_widgets')) {
					...
					if (!gamezone_exists_trx_addons())
						...
						gamezone_customizer_need_widgets_message('front_page_team_caption', 'ThemeREX Addons - Team');
				}
			?></div>
		</div>
	</div>
